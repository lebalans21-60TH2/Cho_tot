...
import { homeProducts,foodCategoryData, DataFood } from "../../../components/Constant/indexData";
...
const page = () => {
  // const category = "food";
  let FilterNormar = DataFood
  // Lọc sản phẩm theo thể loại đồ gia dụng
  // let FoodData = homeProducts.filter((sp) => sp.category === category);
  // const DataProducts = FoodData;
  const Datadirections = DataFood[0];
            ...
            <div className="max-w-[66.666%] p-0 ">
              ...
              <div className="relative">
                <div
                  ...
                  className="overflow-x-auto flex mb-2 p-0 list-none"
                >{
                  foodCategoryData.map((foodCategoryData)=>(
                    <div key={foodCategoryData.id} className="bg-white capitalize inline-flex justify-center items-center mr-2 p-1.5">
                      <div className={`${foodCategoryData.color} w-[94px] text-center text-[13px] overflow-hidden text-[ellipsis] leading-[16px] `}>{foodCategoryData.title}</div>
                    </div>
                  ))
                }
