...
const page = () => {
  ...
  // const category = "serviceTravel";
  let FilterNormar = DataTraverService;
  // Lọc sản phẩm theo thể loại đồ gia dụng
  // let servicetravel = homeProducts.filter((sp) => sp.category === category);

  // test phần filter
  const [products, setProducts] = useState([]);
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [categorySub, setCategorySub] = useState("");
  const [category, setCategory] = useState("serviceTravel");

  // const DataProducts = servicetravel;
  const Datadirections = DataTraverService[0];
