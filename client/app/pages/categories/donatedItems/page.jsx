...
import { CategoriesHouseWareData, DataDonateProducts } from "../../../components/Constant/indexData";
...
const page = () => {
  ...
  const category = "donateProducts";
  let FilterNormar = DataDonateProducts
  // Lọc sản phẩm theo thể loại đồ gia dụng
  let donatedProducts = homeProducts.filter((sp) => sp.category === category);
  const DataProducts = donatedProducts;
  const Datadirections = DataDonateProducts[0];
      ...
      <div className="hidden 800px:w-full bg-[#f4f4f4] 800px:flex 800px:flex-col">
        ...
        >
          {/* Thanh loc thong tin tim kiem */}
            <Filter FilterNormar={FilterNormar} />
          {/* Phần dirrection */}
                ...
                <div className="relative">
                  ...
                  >
                    {SubBannerData.map((SubBannerData) => (
                      <div onClick={()=>router.push(SubBannerData.router)} className="text-center flex min-w-[80px] flex-col cursor-pointer items-center">
                        <div className="bg-[#f4f4f4] w-14 h-14 justify-center rounded-[50%] flex items-center">
                          <span className="box-border inline-block overflow-hidden w-[initial] h-[initial] opacity-100 relative max-w-full m-0 p-0 border-0">
                            <img
                              className="block max-w-full w-[initial] h-[initial] opacity-100 m-0 p-0 border-0"
                        ...
                        <div
                          ...
                          className="whitespace-nowrap text-center text-ellipsis overflow-hidden w-full text-[#222] text-xs mt-2"
                        >
                          {SubBannerData.title}
                        </div>
                      </div>
